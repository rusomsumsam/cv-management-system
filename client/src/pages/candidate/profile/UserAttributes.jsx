...
const UserAttributes = () => {
    ...
    const filteredAttributes = attributes.filter((item) => {
        ...
        return (
            item.attribute?.name?.toLowerCase().includes(searchLower) ||
            item.attribute?.category?.toLowerCase().includes(searchLower) ||
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900">My Attributes</h1>
                            <p className="text-slate-600 mt-1">
                                Manage your profile attribute values used for CV generation.
                            </p>
                        </div>
                        <button
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search by attribute name, category, or value..."
                            value={searchTerm}
                            ...
                            <tbody>
                                {filteredAttributes.length === 0 ? (
                                        ...
                                        <td colSpan="5" className="py-12 text-center">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <p className="text-slate-400 text-xs mt-1">
                                                    Add attributes from the library to complete your profile.
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    filteredAttributes.map((item) => (
                                        <tr
                                            key={item.id}
                                            onClick={() => navigate(`/profile/attributes/${item.id}`)}
                                        ...
                                        >
                                            ...
                                            <td className="py-4 px-6">
                                                {getTypeBadge(item.attribute?.type)}
                                            </td>
